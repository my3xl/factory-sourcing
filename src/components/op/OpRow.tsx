...
export default function OpRow({ op }: OpRowProps) {
  ...
  return (
    <div className="border-b border-brand-border last:border-b-0">
      ...
      >
        <div className="px-5 pb-4 pt-1 flex items-center gap-4 bg-brand-cream/50">
          <span className="text-sm text-brand-warm">
            {t(lang, 'matchingCount', { count: op.matchedCount })}
          </span>
          <button
            onClick={(e) => {
              e.stopPropagation();
            ...
            disabled={refreshing}
            className="text-xs px-3 py-1.5 rounded bg-brand-brown text-white hover:bg-brand-brown/90 disabled:opacity-50 transition-colors flex items-center gap-1"
          >
            {refreshing ? (
              <>
          ...
          {/* Placeholder for future factory list link */}
        </div>
      </div>
    </div>
  );
